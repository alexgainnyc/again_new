...
class Request
{
	...
	public static function pathInfo()
	{
		...
		if (empty($pathInfo) || $pathInfo == "/")
		{
			$pieces = explode("?", array_get(static::$server, "REQUEST_URI", "/"));
			$script = array_get(static::$server, "SCRIPT_NAME");
			$pos = strpos($script, "index.php");
			$pathInfo = "/" . ltrim(substr($pieces[0], $pos), "/");
		}

		if (empty($pathInfo) || $pathInfo == "/")
		{
			$pathInfo = "/" . ltrim(array_get(static::$server, "REDIRECT_QUERY_STRING", "/"), "/");
		}
