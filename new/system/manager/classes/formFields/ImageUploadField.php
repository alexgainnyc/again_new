...
class ImageUploadField extends UploadField
{
		...
		foreach ($files as $file)
		{
			if (array_key_exists("_tmpName", $file))
			{
				$items[] = array(
					...
					"name" => $file["_name"],
					"thumb" => URL::thumb("app/storage/tmp/" . $file["_tmpName"], 100, 75, Image::RESIZE_METHOD_FIT_NO_MARGING)
				);
			}
			else
			{
				...
				$items[] = array(
					...
					"thumb" => URL::thumb("app/storage/" . $first, 100, 75, Image::RESIZE_METHOD_FIT_NO_MARGING)
				);
			}
		}
	...
	/*public function delete($id, $index)
	{
		if ($index >= 0)
		{
			$files = $this->filesList($id, $this->sessionKey);
