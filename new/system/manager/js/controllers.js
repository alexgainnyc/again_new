...
angular.module('manager.controllers', [])
		...
		$scope.send = function(){
			...
			$http.post(config.api_url + 'token/', $scope.form)
				...
				.error(function(error){
					var error_messages = {
						'invalid_client' : 'User or Password invalid.'
					}

					$scope.form.error = error_messages[error.error_description] || error.error_description || "User or Password invalid.";
				})
		}
	}])
	...
	.controller('edit', ['$rootScope', '$scope', '$routeParams', '$location', 'Restangular', '$http', '$timeout', function($rootScope, $scope, $routeParams, $location, Restangular, $http, $timeout) {
		...
		$scope.$watch('uploads', function(uploads, oldValue){
			var uploading = false;

			_.each(uploads, function (item) {
				if (item)
				{
					uploading = true;
				}
		...
		$scope.save = function(model){
			...
			Restangular.restangularizeElement(null, { data: model.data }, table + '/' + model.data.id).put().then(function(){
				...
				save_states['error']['label'] = response.data.error_description || 'Unexpected error';
